...
The clinical authorization policies are the same shape with a worse failure. A policy rewritten to be more readable can stop the patient index being used, or a session setting applied without transaction scope can leak one caller's identity into the next request through a pooled connection. Both look like tidy-ups.
