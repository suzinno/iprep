...
3. **A charge and the thing it charges for.** `connection_request` and the `billing_charge` that accrues from it must not diverge — a charge with no connection is a refund conversation, and a connection with no charge is revenue lost silently. Here the design takes an extra belt: the charge is derived asynchronously from the event, and the schema carries a partial unique index on `(connection_request_id)` `WHERE kind = 'connection'` so at-least-once delivery cannot produce two.
