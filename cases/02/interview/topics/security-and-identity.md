...
- **Policies are written so the patient predicate still reaches the planner.** This keeps partition pruning intact on the monthly-partitioned tables. A policy that hides the key behind an opaque subquery silently turns a pruned index scan into a full sweep. So an `EXPLAIN` assertion guards the plan shape.
...
- **A plan assertion**, because a policy rewritten for readability can hide the partition key from the planner. The rewritten policy can then turn a pruned scan into a full sweep. That failure is about performance, not security. But it is the same class: a change that looks like a tidy-up and is not.
