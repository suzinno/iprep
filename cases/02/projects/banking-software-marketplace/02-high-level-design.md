...
| `identity-service` | OAuth2 authorization server, JWT issuance, refresh rotation, JWKS | A distinct trust boundary; compromise here is categorically worse than elsewhere |

Three Celery worker deployments run alongside them: `catalog-import-worker`, `indexer-worker` and `notification-worker`. They share the application codebase but not the request path, so a large import cannot exhaust web-tier capacity.

**Operational cost, stated plainly.** Six services plus three worker pools is more than 35 QPS requires. The justification is isolation of blast radius and of tenant data, not throughput — and it is only affordable because all nine share one repository, one Alembic migration history, one CI pipeline and one AKS cluster. A team that split these into separate repositories at this scale would spend more on coordination than the boundaries are worth.
...
REST over HTTPS, JSON, versioned at `/v1`. Pydantic models define every request and response body and generate the OpenAPI document that the admin console and vendor integrations build against. Pagination is **keyset (cursor)** everywhere — offset pagination degrades badly on the deep result pages a comparison workflow produces.
...
| `POST /v1/vendor/imports` | multipart CSV/JSON → `blob-media` | `ImportJob { id, status: queued }` |
...
**Admin (scope `platform:admin`, no org scoping)** — `/v1/admin/vendors`, `/v1/admin/retail-groups`, `/v1/admin/stores`, `/v1/admin/products`, each with list/detail/patch and a state-transition endpoint. The admin console is a static SPA served from `blob-media` through Front Door; it calls these same versioned APIs rather than a private backend. The cost is a chattier UI on entity screens that join across services; the benefit is that no admin capability exists which the public API contract does not already describe and test.
...
|---|---|---|
| **Python / FastAPI** | All six services and the worker entrypoints. Async request handling suits a workload dominated by waiting on Postgres, Mongo and Redis; native Pydantic integration makes the OpenAPI contract a build artefact rather than documentation | `02` |
...
| **SQLAlchemy** | ORM and Core for `postgres-core`. Core (not ORM) for the catalog search query, where the generated plan matters | `05` |
...
| **PostgreSQL** | `postgres-core` — the system of record for every entity with relational integrity: vendors, retailers, stores, listing spine, shortlists, connections, billing, audit. Also the search projection | `03` |
| **MongoDB** | `mongo-catalog` — per-category product metadata with no fixed column set, plus immutable revisions and import staging | `03` |
...
| **OAuth2** | Authorization code + PKCE for humans, client credentials for vendor integrations; JWT RS256 access tokens with `act` (account type) and `org_id` claims | `06` |
...
| **Kubernetes (AKS)** | Runtime for all services and workers; HPA on CPU and on Celery queue depth | `05` |
| **Terraform** | All Azure infrastructure — AKS, Postgres, Redis, Blob, Service Bus, Front Door, APIM, Key Vault — with state in a locked Blob container | `05` |
...
| **Azure Functions** | `fn-media-process` (blob-triggered thumbnail/PDF preview generation) and `fn-notify-dispatch` (queue-triggered email/webhook delivery) | `04` |
...
**Two flagged additions outside the brief's stack.** *Azure Key Vault* for CMK and secret material, because "Azure … etc." cannot reasonably be read to exclude it and no listed component can hold a key safely. *Azure Front Door with WAF*, because APIM provides quotas and routing but not a web application firewall or L7 DDoS absorption; `06-security.md` depends on it. Both are named here so no later file introduces them silently.

## Alternatives Considered and Rejected

| Considered | Chosen instead | Reasoning |
|---|---|---|
| Elasticsearch / Azure AI Search for catalog search | PostgreSQL projection table with GIN indexes | 40,000 listings with structured facets is far inside what Postgres handles well. A search cluster adds a third data store, a second consistency lag and real operational expertise for relevance ranking nobody has asked for. The evolution trigger is stated in `01-requirements.md` |
...
| Azure Cosmos DB for NoSQL | MongoDB (deployable as a replica set or as Cosmos DB for MongoDB vCore) | The brief specifies MongoDB. Cosmos DB for NoSQL is a different API and would rewrite every query |
| gRPC between services | REST + JSON internally | Nine deployments at this traffic level gain nothing from binary framing, and REST keeps one contract style and one test approach across internal and external surfaces |
| Kafka / Event Hubs | Azure Service Bus | Event volume is ~0.2/s. Service Bus topics give competing consumers, dead-lettering and native Functions triggers with no cluster to run |
| Service mesh (Istio/Linkerd) for mTLS | NetworkPolicy plus TLS to data stores | Discussed with its trigger in `06-security.md` — the mesh's operational cost is disproportionate to nine workloads in one namespace |
