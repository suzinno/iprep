...
The brief describes the platform once as "a marketplace for banking software solutions" and then, in every other sentence and in every responsibility, as a marketplace for retail operations software — POS, inventory, and loyalty tools sold to brick-and-mortar and chain retail. This design follows the body of the brief: the product is a **retail software marketplace**, and financial-services software (payment terminals, reconciliation, retail banking integrations) is one vendor **category** within it rather than the platform's subject.
...
**B2B, three-sided, with an operator role.** No consumer surface exists.
...
4. **Vendor–retailer connection and threaded conversation.** A connection request from a listing becomes a durable thread; this replaces the separate RFP round.
...
| **Latency — listing detail** | p95 < 150 ms | Redis-served on the hot path |
| **Latency — connection create** | p95 < 300 ms | Synchronous commit plus outbox insert; delivery is async |
| **Scalability** | 100 QPS peak sustained without re-architecture | Roughly 3× the modelled peak; see the evolution triggers below |
| **Consistency (CAP)** | **CP for connections, billing and identity; AP for catalog reads** | A connection request, a charge or a token must never be lost or double-counted. A listing edit that takes seconds to appear in search results costs nothing |
...
| **Durability** | RPO 15 min, RTO 4 h | Geo-redundant backup, not a warm standby region |
...
- **DAU 3,000**, MAU 12,000 — weekday-concentrated B2B usage, roughly 12% of registered users active daily.
- ~120 API requests per active user per day → **≈360,000 requests/day ≈ 4.2 QPS average**.
...
- **No payment processing between retailer and vendor.** The platform brokers a conversation; contracts and money settle off-platform. Vendor subscription billing is in scope and is delegated to an external PSP, which keeps cardholder data entirely out of the system (`06-security.md`).
