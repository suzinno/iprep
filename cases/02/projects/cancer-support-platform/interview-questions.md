...
*Ordering.* Check-ins are independent per patient, so parallelism is free. Attribute updates for the same product must apply in order or the final state is wrong. That means partitioning by entity key and keeping one consumer per partition — the point where a log-structured broker such as Kafka starts to look more natural than RabbitMQ, since partitioned ordered consumption is its native model rather than something to construct.
