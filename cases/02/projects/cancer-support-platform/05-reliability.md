...
| Check-ins over a date window | `GET /api/v1/diary/check-ins` | Monthly range partition on `recorded_for` + [BRIN](https://www.postgresql.org/docs/current/brin.html "Block Range Index — Compact PostgreSQL index type suited to large, sequentially correlated tables") on `recorded_at`; unique `(patient_id, recorded_for)` |
