...
Microsoft's cloud identity service, called Azure Active Directory until 2023. It holds an organisation's users, groups and applications, signs people and services in, and issues the tokens that Microsoft 365, Azure and any application registered with it accept as proof of who is calling.

<details><summary>Details</summary>

**How it works:** Everything lives in a *tenant* — one organisation's instance, identified by a tenant ID and a domain such as `contoso.onmicrosoft.com` — whose user and group objects are read and written through the Microsoft Graph REST API rather than through any directory protocol. An application becomes known to it through an *app registration*: a client ID, the redirect URIs it may use, a credential if it is a confidential client, the scopes it exposes and any *app roles* it defines. Its working instance inside a tenant is a *service principal*, the object that role assignments, consent and sign-in logs attach to — so a multi-tenant application has one registration in its home tenant and a service principal in every tenant that consents to it. Sign-in speaks the open protocols: OAuth 2.0 and OIDC through `login.microsoftonline.com/<tenant>/oauth2/v2.0/authorize`, and SAML 2.0 for the many enterprise applications that integrate that way. Tokens are JWTs whose *claims* — named fields — include `tid` for the tenant, `oid` for the signed-in object, `aud` for the intended recipient, and `scp` for delegated scopes or `roles` for app roles assigned to the user or to a calling service; by default they are signed with keys Microsoft publishes for every tenant alike, so a valid signature proves Entra issued a token, not which tenant it came from. Before a token is issued, *Conditional Access* policies evaluate the sign-in — who, to which application, from which location or device — and can require MFA, require a compliant device or block it outright; the feature needs a P1 licence or above. Azure workloads avoid stored secrets through *managed identities*: the resource is given a service principal whose credential Azure holds and rotates, and the code asks a local platform endpoint for a token. An organisation still running on-premises Active Directory synchronises its users into the tenant with Microsoft Entra Connect or Cloud Sync, keeping the on-premises directory as the source.

**Boundary:** Not Active Directory moved to the cloud, whatever the old name suggests. It answers no LDAP query and has no organisational units or Group Policy, so an application that binds over LDAP or expects to join a domain needs Microsoft Entra Domain Services — a separate, paid managed domain populated from the tenant — or a domain controller kept on premises. Its *directory roles*, such as Global Administrator, govern the tenant; access to Azure subscriptions and resources is Azure RBAC — Owner, Contributor, Reader, assigned at a scope — a separate authorization system that only takes its identities from Entra ID, which is why a Global Administrator holds no rights over a subscription by default. Microsoft Entra names a product family, and Entra ID is the workforce directory at its centre: sign-up and sign-in for a public app's customers is Microsoft Entra External ID, the successor to Azure AD B2C. Another identity provider such as Okta can take over sign-in for a Microsoft 365 organisation only by federating into its tenant, which remains the directory underneath. And disabling a user does not recall an access token already issued, as with any OAuth 2.0 deployment; here that means up to its default lifetime of 60 to 90 minutes, unless both the client and the resource support Continuous Access Evaluation.

**Alternatives:**
- **Okta Workforce Identity** — an independent identity provider with a broad catalogue of prebuilt application integrations, often chosen where the estate is not mostly Microsoft. Neutral across vendors, at the cost of the built-in integration with Microsoft 365 and Azure that Entra ID starts with.
- **Keycloak** — an open-source identity provider you host yourself, speaking the same OIDC and SAML. No per-user licence and full control of the data, in exchange for operating, patching and securing it yourself.

**Example:** An internal expense application is registered in the tenant with two app roles, `Submitter` and `Approver`, and the finance group is assigned `Approver`. A Conditional Access policy requires MFA for that application outside the office network, so an approver signing in from home is prompted before any token is issued. The application's API then receives an access token and checks three claims rather than the signature alone: `aud` names its own registration, `tid` is the company's tenant, and `roles` contains `Approver`. A valid signature alone would have accepted a token Entra issued to any application in any tenant.
